 ...
 * 一：代码重构之Lambda化：
 *      1.进进出出、摇摇晃晃：比如日志打印
 *      2.孤独的覆盖。如果继承的目的只为了覆盖一个方法，则可以用lambda替换。
 *      3.同样的东西写两遍，逻辑通用，代表的含义不通。比如计算有一个Order类，计算
 *        音乐家数、取目和歌曲时长。可参考toLongFunction
 *
 * 二：测试Lambda表达式：
 *      1.将lambda中复杂的逻辑放到方法中，在lambda表达式中使用方法引用：List::size(),单元测试只要测试size方法即可。
 *      2.测试替身，也称模拟，Mock框架大部分语言都支持，JAVA也不例外
 *
 * 三：调试Lambda表达式：
 *      1.可以将逻辑放到方法中，和非lambda代码正常调试并无差异。
 *      2.使用peek()方法进行调试，还可以将数据写入到指定的日志中。
 */
public class DubugAndRefactoryDemo {
